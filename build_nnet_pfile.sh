...
# Create pfiles for deep neural network training. Assumes the training alignment is ready
# and features are (LDA+MLLT or delta+delta-delta) + fMLLR
#
# 9 frames (+4, -4) of LDA+MLLT+fMLLR are spliced. This gives the dimension of 360 (if 
# we use the default LDA dim of 40).
# Optionally, an LDA is further estimated and applied to spliced fMLLR. This projects the
# spliced features from the dimension of 360 to 250 (by default)
